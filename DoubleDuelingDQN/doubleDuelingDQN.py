...
class DoubleDuelingDQN(AgentWithConverter):
    ...
    def quantizated_act(self, state, reward, done=False):
        # Register current state to stacking buffer
        self._save_current_frame(state)
        # We need at least num frames to predict
        if len(self.frames) < self.num_frames:
            return 0  # Do nothing
        # Infer with the last num_frames states
        data = np.array(self.frames, dtype=np.float32)
        model_input = data.reshape(1, self.observation_size * self.num_frames)

        self.Qmain.interpreter.set_tensor(self.Qmain.input_details[0]['index'], model_input)
        self.Qmain.interpreter.invoke()
        # The function `get_tensor()` returns a copy of the tensor data.
        # Use `tensor()` in order to get a pointer to the tensor.
        output_data = self.Qmain.interpreter.get_tensor(self.Qmain.output_details[0]['index'])
        opt_policy = np.argmax(output_data)
        return opt_policy
    ...
    def act(self, observation, reward, done=False):
        if not self.is_quantizated:
            ...
            encoded_act = self.quantizated_act(transformed_observation, reward, done)
            print("========quantization act========")
