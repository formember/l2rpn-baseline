...
try:
    ...
    import tensorflow.compat.v1.keras.optimizers as tfko
    import tensorflow.compat.v1.keras.layers as tfkl
    import tensorflow.compat.v1.keras.activations as tfka
...
class DoubleDuelingDQN_NN(object):
    ...
    def __init__(self,
        ...
        self.is_quantizated=is_quantizated
    ...
    def predict_move(self, data):
        ...
        if self.is_quantizated:
            converter = tf2.lite.TFLiteConverter.from_keras_model(self.model)
            converter.optimizations = [tf2.lite.Optimize.DEFAULT]
            tflite_quant_model = converter.convert()
            tflite_name = "tflite_model.tflite"
            open(tflite_name, "wb").write(tflite_quant_model)
            interpreter = tf2.lite.Interpreter(model_path="tflite_model.tflite")
            ...
            interpreter.invoke()
            q_actions = interpreter.get_tensor(output_details[0]['index'])
        else:
    ...
    def load_network(self, path):
        ...
        if not self.is_quantizated:
            ...
            print("Successfully loaded quantizated network from: {}".format(path))
